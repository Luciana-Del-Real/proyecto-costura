  ...
  return (
    <div id={`lesson-${lesson.id}`} className={`bg-white border rounded-2xl shadow-sm overflow-hidden transition-colors ${isOpen ? 'border-secondary' : 'border-border'}`}>
      ...
      <button
        ...
        disabled={blocked}
        className={`w-full flex items-center gap-3 p-4 lg:p-5 text-left ${blocked ? 'opacity-60 cursor-not-allowed' : 'hover:bg-bg-soft/60'} transition-colors ${isOpen ? 'bg-primary-soft/40' : ''}`}
      >
