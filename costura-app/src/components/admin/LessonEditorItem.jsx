...
// se muestra colapsada como acordeón (título + duración) para scrollear
// fácil a la próxima; al expandir muestra los campos editables, PDFs a
// subir/ya subidos y botones de guardar/eliminar.
export default function LessonEditorItem({
  lesson, isOpen, onToggle, editedLessons, onFieldChange, onLessonPdfChange,
    ...
    <div id={`lesson-${lesson.id}`} className={`bg-white border rounded-2xl shadow-sm overflow-hidden transition-colors ${isOpen ? 'border-secondary' : 'border-border'}`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-text-ink truncate">{getLessonField('title') || 'Lección sin título'}</p>
          <p className="text-xs text-accent mt-0.5 flex items-center gap-1">
            <Clock className="w-3.5 h-3.5" strokeWidth={1.5} /> {getLessonField('duration') || 'Sin duración'}
          </p>
        </div>
        <ChevronDown className={`w-4 h-4 text-text-ink flex-shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`} strokeWidth={2} />
      </button>
